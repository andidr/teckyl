...
using IteratorBoundsMap = std::map<std::string, std::pair<mlir::Value, mlir::Value>>;
...
public:
  ...
  mlir::FuncOp buildFunction(const std::string& name,
			     const lang::Def& def)
  {
    ...
    mlir::FuncOp funcOp = mlir::FuncOp::create(loc(def.range()),
					       name,
					       func_type);
...
private:
  ...
  mlir::FileLineColLoc loc(const lang::SourceRange& r) {
    return builder.getFileLineColLoc(builder.getIdentifier(filename),
				     r.startLine(),
				     r.endLine())
  ...
  OpTy buildBinaryExpr(const lang::TreeRef& t)
  {
    ...
    if(vals[0].getType() != vals[1].getType()) {
	 ...
	 << getTypeAsString(vals[0].getType())
	 << " and "
	 << getTypeAsString(vals[1].getType());
  ...
  mlir::Value buildConstant(const lang::Const& cst)
  {
	...
	return builder.create<mlir::ConstantIntOp>(location,
						   (int64_t)cst.value(),
						   num_bits);
      } else {
	throw mlirgen::SourceException
	  (location, "Unsigned integers are not supported");
      }
    } else if(isFloatType(kind)) {
      mlir::FloatType floatType = getFloatType(kind);

      return builder.create<mlir::ConstantFloatOp>(location,
						   llvm::APFloat(cst.value()),
						   floatType);
    } else {
  ...
  mlir::StoreOp buildIndexStoreExpr(mlir::Value& valueToStore,
				    const lang::Ident& ident,
				    const lang::ListView<lang::Ident>& indices)
  {
    ...
    mlir::StoreOp ret = builder.create<mlir::StoreOp>
      (location, valueToStore, tensor, argVals);
  ...
  // stored in `*innermost`.
  mlir::AffineForOp buildAffineLoopNest(const std::vector<std::string>& iterators,
					...
					mlir::AffineForOp* innermost = nullptr)
  {
	...
	(
	 location,
	 mlir::ValueRange({ mlirIteratorBounds.at(it).first }),
	 mlir::AffineMap::get(0, 1, { mlir::getAffineSymbolExpr(0, builder.getContext())}),
  ...
  translateIteratorBounds(const IteratorRangeMap& langBounds)
  {
    ...
    for(const std::pair<std::string, lang::RangeConstraint>& langBound : langBounds) {
  ...
  void buildComprehension(const lang::Comprehension& c)
  {
	...
	mlir::AffineForOp outermostInit =
	  buildAffineLoopNest(iteratorsSeq, mlirItBounds,
			      startLoc, &innermostInit);
	...
	if(outTensorElementType.isa<mlir::FloatType>()) {
	  mlir::FloatType fType =
	    outTensorElementType.cast<mlir::FloatType>();
	  ...
	  cstVal = builder.create<mlir::ConstantIntOp>
	    (startLoc, (int64_t)cst, iType.getWidth());
	} else {
	...
	(startLoc,
	 buildIndexLoadExpr(c.ident(), c.indices()),
	 rhsVal);
      break;
