...
struct TreeView {
  ...
  TreeRef tree() const { return tree_; }
  const SourceRange &range() const { return tree_->range(); }
...
template <typename T> struct ListViewIterator {
  ListViewIterator(TreeList::const_iterator it) : it(it) {}
  bool operator!=(const ListViewIterator &rhs) const { return it != rhs.it; }
...
template <typename T> struct ListView : public TreeView {
  ListView(const TreeRef &tree) : TreeView(tree) { tree->expect(TK_LIST); }
  typedef ListViewIterator<T> iterator;
  typedef ListViewIterator<T> const_iterator;
  iterator begin() const { return iterator(tree_->trees().begin()); }
  iterator end() const { return iterator(tree_->trees().end()); }
  T operator[](size_t i) const { return T(tree_->trees().at(i)); }
  TreeRef map(std::function<TreeRef(const T &)> fn) {
    return tree_->map([&](TreeRef v) { return fn(T(v)); });
  }
  size_t size() const { return tree_->trees().size(); }
...
template <typename T> struct OptionView : public TreeView {
  explicit OptionView(const TreeRef &tree) : TreeView(tree) {
    tree->expect(TK_OPTION);
  }
  bool present() const { return tree_->trees().size() > 0; }
  T get() const {
...
struct Ident : public TreeView {
  ...
  const std::string &name() const { return subtree(0)->stringValue(); }
...
template <int kind> struct ApplyLike : public TreeView {
  explicit ApplyLike(const TreeRef &tree) : TreeView(tree) {
    tree_->expect(kind, 2);
  }

  Ident name() const { return Ident(subtree(0)); }
  ListView<TreeRef> arguments() const { return ListView<TreeRef>(subtree(1)); }
...
struct BuiltIn : public TreeView {
  ...
  TreeRef type() const { return subtree(2); }

  static TreeRef create(const SourceRange &range, const std::string &name,
                        TreeRef arguments, TreeRef type) {
    return Compound::create(TK_BUILT_IN, range,
                            {String::create(name), arguments, type});
  }
...
struct TensorType : public TreeView {
  ...
  static TreeRef create(const SourceRange &range, TreeRef scalar_type_,
                        TreeRef dims_) {
  ...
  int scalarType() const { return scalarTypeTree()->kind(); }
  // either an Ident or a constant
  ListView<TreeRef> dims() const { return ListView<TreeRef>(subtree(1)); }
};
...
struct Param : public TreeView {
  ...
  // parameter.
  Ident ident() const { return Ident(subtree(0)); }
  // may be TensorType or TK_INFERRED
  TreeRef type() const { return subtree(1); }
  bool typeIsInferred() const { return type()->kind() == TK_INFERRED; }
  // helper for when you know the type is not inferred.
  TensorType tensorType() const { return TensorType(type()); }
};
...
struct Equivalent : public TreeView {
  ...
  static TreeRef create(const SourceRange &range, const std::string &name,
                        TreeRef accesses) {
    return Compound::create(TK_EQUIVALENT, range,
                            {String::create(name), accesses});
  }
  const std::string &name() const { return subtree(0)->stringValue(); }
  ListView<TreeRef> accesses() const { return ListView<TreeRef>(subtree(1)); }
};
...
struct RangeConstraint : public TreeView {
  ...
  static TreeRef create(const SourceRange &range, TreeRef ident, TreeRef start,
                        TreeRef end) {
    return Compound::create(TK_RANGE_CONSTRAINT, range, {ident, start, end});
  }
  Ident ident() const { return Ident(subtree(0)); }
  TreeRef start() const { return subtree(1); }
  TreeRef end() const { return subtree(2); }
};
...
struct Comprehension : public TreeView {
  ...
  static TreeRef create(const SourceRange &range, TreeRef ident,
                        TreeRef indices, TreeRef assignment, TreeRef rhs,
                        TreeRef range_constraints, TreeRef equivalent,
                        TreeRef reduction_variables) {
    return Compound::create(TK_COMPREHENSION, range,
                            {ident, indices, assignment, rhs, range_constraints,
                             equivalent, reduction_variables});
  }
  // when the type of a field is statically know the accessors return
  // the wrapped type. for instance here we know ident_ is an identifier
  // so the accessor returns an Ident
  ...
  // parameter.
  Ident ident() const { return Ident(subtree(0)); }
  ListView<Ident> indices() const { return ListView<Ident>(subtree(1)); }
  // kind == '=', TK_PLUS_EQ, TK_PLUS_EQ_B, etc.
  TreeRef assignment() const { return subtree(2); }
  TreeRef rhs() const { return subtree(3); }
...
struct Def : public TreeView {
  ...
  static TreeRef create(const SourceRange &range, TreeRef name,
                        ...
                        TreeRef stmts_list) {
    return Compound::create(TK_DEF, range,
...
struct Select : public TreeView {
  ...
  Ident name() const { return Ident(subtree(0)); }
  int index() const { return std::stoi(subtree(1)->numValue()); }
...
struct Const : public TreeView {
  ...
  const std::string &value() const { return subtree(0)->numValue(); }
...
struct Cast : public TreeView {
  ...
  TreeRef value() const { return subtree(0); }
  TreeRef type() const { return subtree(1); }
  static TreeRef create(const SourceRange &range, TreeRef value, TreeRef type) {
...
struct Let : public TreeView {
  ...
  Ident name() const { return Ident(subtree(0)); }
  TreeRef rhs() const { return subtree(1); }
...
struct Exists : public TreeView {
  ...
  TreeRef exp() const { return subtree(0); }
